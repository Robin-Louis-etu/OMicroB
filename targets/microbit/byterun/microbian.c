...
struct _proc {
    ...
    message* msg;         /* Pointer to message buffer */
...
{
    if (htop - hbot < (int) sizeof(struct _proc))
...
static void microbian_dump(void)
{
    ...
    for (int pid = 0; pid < (int) os_nprocs; pid++) {
...
static void await_reply(proc pdst, message *msg)
{
    ...
    if (psrc != NULL) {
        ...
        deliver(pdst->msg, psrc->pid, REPLY, msg);
        make_ready(pdst);
        make_ready(psrc);
    } else {
...
static void mini_send(int dest, int type, message *msg)
{
    ...
    if (dest < 0 || dest >= (int) os_nprocs || pdest->state == DEAD)
    ...
    if (accept(pdest, type)) {
        ...
        deliver(pdest->msg, src, type, msg);
        make_ready(pdest);
...
static void mini_receive(int type, message *msg)
{
    ...
    if (type != INTERRUPT) {
        ...
        if (psrc != NULL) {
            deliver(msg, psrc->pid, psrc->msgtype, psrc->msg);
            make_ready(os_current);
...
void interrupt(int dest)
{
    ...
    if (accept(pdest, INTERRUPT)) {
        ...
        deliver(pdest->msg, HARDWARE, INTERRUPT, NULL);
...
static proc create_proc(const char *name, unsigned stksize)
{
    int pid;
    proc p;
    unsigned char *stack;
    ...
    stack = (unsigned char *) sbrk(stksize);
...
int start(const char *name, void (*body)(int), int arg, int stksize)
{
...
void __start(void)
{
    microbit_write_pixel(2, 2, 200);
    microbit_write_pixel(3, 3, 0);
    //display_task();
